...
import { Loader2, Save, Trash, Search } from "lucide-react";
...
export default function DeckBuilderPage() {
  ...
  const deckId = params.id as Id<"userDecks">;

  // Data Fetching
  const deckData = useQuery(api.lunchtableTcgCards.component.decks.getDeckWithCards, { deckId });
  const allCards = useQuery(api.lunchtableTcgCards.component.cards.getAllCards);

  // Mutations
  const saveDeck = useMutation(api.lunchtableTcgCards.component.decks.saveDeck);
  const renameDeck = useMutation(api.lunchtableTcgCards.component.decks.renameDeck);
  ...
  const handleAddCard = (card: any) => {
    ...
    setCurrentDeck(prev => {
      ...
      if (existing) {
        ...
        return prev.map(c => c.cardDefinitionId === card._id ? { ...c, quantity: c.quantity + 1 } : c);
      }
  ...
  const handleRemoveCard = (cardId: string) => {
    setCurrentDeck(prev => {
      ...
      if (existing.quantity > 1) {
        return prev.map(c => c.cardDefinitionId === cardId ? { ...c, quantity: c.quantity - 1 } : c);
      }
      ...
      <header className="flex items-center justify-between p-4 border-b bg-background z-10">
        <div className="flex items-center gap-4 flex-1">
          ...
          <div className="flex gap-2 text-sm font-mono text-muted-foreground">
             <span className={cn(totalCards < 30 ? "text-yellow-600" : totalCards > 60 ? "text-red-600" : "text-green-600")}>
                {totalCards}
             </span>
             / 60 Cards
          </div>
            ...
            <ScrollArea className="flex-1 p-4">
                <div className="grid grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
                    {filteredLibrary.map((card: any) => {
                        const inDeck = currentDeck.find(c => c.cardDefinitionId === card._id)?.quantity || 0;
                        return (
