  ...
  return (
    <div className="relative h-screen w-screen overflow-hidden bg-black">
      ...
      <div className="absolute inset-0 z-0 opacity-50">
        <Image src={ASSETS.boardBg} alt="Board Background" fill className="object-cover" priority />
        ...
        <div className="flex flex-col items-center gap-2">
          ...
          <div className="flex gap-[-2rem] justify-center">
            {gameState.opponentHand.map((_card: any, i: number) => (
              <div
                key={i}
                className="relative w-24 h-36 bg-slate-800 border-2 border-slate-600 rounded-lg shadow-xl translate-y-[-2rem] hover:translate-y-0 transition-transform"
              >
                {/* Card Back */}
          ...
          <div className="flex gap-4 p-4 bg-white/5 rounded-xl border border-white/10 backdrop-blur-sm">
            ...
            <div className="flex gap-2">
              {gameState.opponentSpellTrapZone.map((card: any, i: number) => (
                <div
                  key={`opp-st-${i}`}
                  className="w-20 h-28 border border-dashed border-white/20 rounded flex items-center justify-center"
                >
            ...
            <div className="flex gap-2">
              {gameState.opponentBoard.map((card: any, i: number) => (
                <div
                  key={`opp-m-${i}`}
                  className="w-20 h-28 border border-white/40 rounded flex items-center justify-center bg-red-900/20"
                >
                  {card.isFaceDown ? "Set" : card.name || "Empty"}
                </div>
              ))}
        ...
        <div className="flex flex-col items-center gap-2">
          ...
          <div className="flex gap-4 p-4 bg-white/5 rounded-xl border border-white/10 backdrop-blur-sm">
            ...
            <div className="flex gap-2">
              {gameState.hostBoard.map((card: any, i: number) => (
                <div
                  key={`host-m-${i}`}
                  className="w-20 h-28 border border-white/40 rounded flex items-center justify-center bg-blue-900/20 cursor-pointer hover:bg-blue-800/40 transition-colors"
                >
                  {card.name || "Empty"}
                </div>
              ))}
            ...
            <div className="flex gap-2">
              {gameState.hostSpellTrapZone.map((card: any, i: number) => (
                <div
                  key={`host-st-${i}`}
                  className="w-20 h-28 border border-dashed border-white/20 rounded flex items-center justify-center cursor-pointer"
                >
                  {card.name || "Empty"}
                </div>
              ))}
          ...
          <div className="flex gap-[-1rem] justify-center translate-y-4 hover:translate-y-0 transition-transform duration-300">
            {gameState.hostHand.map((card: any, _i: number) => (
              <div
                key={card._id}
                className="relative w-32 h-48 bg-slate-100 border-2 border-slate-300 rounded-lg shadow-2xl hover:scale-110 hover:z-50 transition-all cursor-pointer -ml-8 first:ml-0"
              >
                <div className="p-2 text-xs font-bold text-black overflow-hidden h-full">
                  {card.name}
                  <div className="mt-1 text-[10px] font-normal">
                    {card.abilityText?.slice(0, 50)}...
                  </div>
                </div>
              </div>
            ))}
