...
export const getWorkpoolStatus = internalQuery({
  ...
  returns: v.object({
    batchOperations: v.object({
      ...
      status: v.union(v.literal("healthy"), v.literal("busy"), v.literal("backlogged")),
    }),
    migrations: v.object({
      queueDepth: v.number(),
      activeJobs: v.number(),
  ...
  handler: async (_ctx) => {
    ...
    const createPoolStatus = (queueDepth: number, activeJobs: number, maxParallelism: number) => {
      const utilizationPercent =
        maxParallelism > 0 ? Math.round((activeJobs / maxParallelism) * 100) : 0;
  ...
  handler: async (_ctx) => {
    ...
    const status = {
      batchOperations: {
        ...
        status: "healthy" as "healthy" | "busy" | "backlogged",
      },
      migrations: {
        queueDepth: 0,
        activeJobs: 0,
    ...
    if (status.migrations.status === "backlogged") {
      warnings.push(`Migrations pool is backlogged (${status.migrations.queueDepth} jobs queued)`);
    }
