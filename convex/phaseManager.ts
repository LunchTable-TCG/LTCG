 ...
 * Records phase_changed event.
 * Auto-advances through non-interactive phases (standby, battle_end).
 */
export const advancePhase = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    // 7. Update game state with new phase
    await ctx.db.patch(gameState._id, {
      currentPhase: nextPhase,
    });

    // 8. Record phase_changed event
    ...
    await ctx.runMutation(api.gameEvents.recordEvent, {
      ...
      description: `${username} entered ${getPhaseDisplayName(nextPhase)}`,
      metadata: {
    ...
    // 9. Execute phase-specific logic
...
async function executePhaseLogic(
  ...
  switch (phase) {
    ...
    case "standby":
      ...
      // Note: This would be handled by client or auto-advance logic
      break;
