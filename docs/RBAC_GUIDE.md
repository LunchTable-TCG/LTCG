...
Deploy the updated schema with the new role types:
...
Run the migration to update existing admin records:
...
### Step 3: Promote First Superadmin

Promote an existing admin to superadmin (bootstrap):
...
Check that all roles are properly set:

```typescript
const admins = await ctx.runQuery(api.admin.roles.listAdminsByRole, {});
console.log(admins);
```
...
# Run with coverage
bun test --coverage convex/lib/roles.test.ts
```
...
- [ ] Moderator can view and manage reports
- [ ] Moderator cannot delete users
- [ ] Admin can delete users
- [ ] Admin can grant moderator roles
- [ ] Admin cannot grant admin roles
- [ ] Superadmin can grant all roles
- [ ] Superadmin can execute cleanup operations
...
### Missing grantedBy in adminRoles

Run the migration script to populate missing `grantedBy` fields:
...
- `/convex/lib/roles.ts` - Core RBAC functions
- `/convex/admin/roles.ts` - Role management mutations
- `/convex/admin/mutations.ts` - Admin operations
- `/convex/social/reports.ts` - Moderation system
