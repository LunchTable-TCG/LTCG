...
import { XP_PER_LEVEL } from "../../lib/storyConstants";
import { calculateLevel, getLevelProgress, getXPForNextLevel } from "../../lib/xpHelpers";
...
describe("hasReachedLevel", () => {
  it("should return true when level reached", async () => {
    ...
    const result = await t.run(async (ctx: MutationCtx) => {
      const { hasReachedLevel } = await import("../../lib/xpHelpers");
