...
export const TEST_ENV = {
  ...
  PRIVY_APP_ID:
    process.env["NEXT_PUBLIC_PRIVY_APP_ID"] || "cml0fnzn501t7lc0buoz8kt74",

  /** Whether tests are running in test mode */
  TEST_MODE: true,

  /** Timeout for network operations (ms) */
  NETWORK_TIMEOUT: parseInt(process.env["TEST_NETWORK_TIMEOUT"] || "30000", 10),

  /** Timeout for page loads (ms) */
  PAGE_LOAD_TIMEOUT: parseInt(
    process.env["TEST_PAGE_LOAD_TIMEOUT"] || "60000",
    10
  ),
...
export function validateTestEnv() {
  ...
  if (missing.length > 0) {
    console.warn(
      `[E2E Test Setup] Missing env vars (using defaults): ${missing.join(", ")}`
    );
  }
