...
export interface TokenLaunchApprovalData {
  _id: Id<"launchApprovals">;
  userId: Id<"users">;
  username: string;
  approved: boolean;
  comments?: string;
...
/** Token launch checklist item */
export interface TokenLaunchChecklistItem {
  _id: Id<"launchChecklist">;
  category: "treasury" | "token" | "marketing" | "technical" | "team";
  item: string;
  description?: string;
  isRequired: boolean;
  isCompleted: boolean;
...
/** Token launch config */
export interface TokenLaunchConfigData {
  _id: Id<"tokenConfig">;
  name: string;
  symbol: string;
  description: string;
...
export interface TokenLaunchApprovalsQueries {
  getAll: TypedQuery<EmptyArgs, TokenLaunchApprovalData[]>;
  getMyApproval: TypedQuery<EmptyArgs, TokenLaunchApprovalData | null>;
  getSummary: TypedQuery<
    EmptyArgs,
    {
      ...
      currentUserApproval: TokenLaunchApprovalData | null;
...
export interface TokenLaunchChecklistQueries {
  ...
  >;
  isReadyForLaunch: TypedQuery<
    EmptyArgs,
    { ready: boolean; incompleteItems: TokenLaunchChecklistItem[] }
  >;
}
...
export interface TokenLaunchConfigQueries {
  getConfig: TypedQuery<EmptyArgs, TokenLaunchConfigData | null>;
  getByStatus: TypedQuery<
    { status: "draft" | "ready" | "launched" | "graduated" } & BaseArgs,
    TokenLaunchConfigData | null
  >;
  getReadiness: TypedQuery<
    EmptyArgs,
    {
      ready: boolean;
      issues: string[];
      checklist: TokenLaunchChecklistItem[];
      config?: TokenLaunchConfigData;
    }
  >;
}
...
export interface TokenLaunchScheduleQueries {
  getSchedule: TypedQuery<
    EmptyArgs,
    { status: string; scheduledAt: number | null; countdown: LaunchCountdown | null }
  >;
  getStatus: TypedQuery<
    EmptyArgs,
    {
      ...
      tokenStatus: string;
      checklist: { total: number; completed: number; percentComplete: number };
      approvals: { total: number; approved: number; required: number };
      config: TokenLaunchConfigData | null;
      isReady: boolean;
      canLaunch: boolean;
    }
  >;
