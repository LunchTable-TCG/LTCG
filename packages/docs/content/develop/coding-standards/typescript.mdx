...
description: TypeScript coding standards and best practices for LTCG
lastUpdated: '2026-02-03'
---

# TypeScript Standards

LTCG project rules for TypeScript development.
...
Use the `returns` validator instead of TypeScript return types:
...
Only annotate when the return type isn't obvious from the implementation:
...
When creating files using Convex, avoid TypeScript depth errors:
...
Never inline `api as any` in components (causes TS2589):

```typescript
// ❌ Avoid
const result = useQuery(api.namespace.function as any);

// ✅ Use helper
const result = useConvexQuery(apiAny.namespace.function);
```
...
```typescript
// ❌ Avoid
const result = aggregate.count(ctx, { namespace: id, bounds: {} });

// ✅ Use
const opts = { namespace: id, bounds: {} };
const result = aggregate.count(ctx, opts);
```

#### Common Errors

- **TS2589** ("Type instantiation excessively deep"): Use helper or extract to module scope
- **TS2304** ("Cannot find name 'apiAny'"): Import from helper

#### Helper Implementation

Ensure helper exists at `lib/convexHelpers.ts`:
...
export const useConvexMutation = (path: any) => useMutation(path);
export const useConvexQuery = (path: any, args?: any) => useQuery(path, args);
export const useConvexAction = (path: any) => useAction(path);
```
